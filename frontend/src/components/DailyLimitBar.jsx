...
export default function DailyLimitBar({ profile, compact = false, className = '' }) {
  ...
  const capRub =
    profile?.monthly_cap_rub ??
    profile?.monthly_quota_rub ??
    profile?.daily_cap_rub;
  ...
  const spentRub =
    profile?.monthly_spent_rub ??
    profile?.daily_spent_rub ??
    (profile?.monthly_spent_usd ?? profile?.daily_spent_usd ?? 0) * rate;
  const subRemainingRub =
    profile?.monthly_remaining_rub ??
    profile?.daily_remaining_rub ??
    Math.max(0, subCap - spentRub);
