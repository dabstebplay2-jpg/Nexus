...
export default function ConnectorsPage({ compact = false }) {
  ...
  const { connectors, loading, error, busyId, connect, disconnect, toggleChat } = useConnectors({
    enabled: authorized,
  });
      ...
      <header className={compact ? 'mb-4' : 'mb-8'}>
        ...
        {authorized && (
          <p className="text-xs text-amber-200/80 mt-2 max-w-2xl">
            Для вызова инструментов в чате выбирайте модели с поддержкой tool calling.
          </p>
        )}
      </header>
